...
{
  Given random generated array A, count all of it's elements.
...
begin
  // Generate array A using following methods:
  // Firstly randomize seed for random method
  // Then run a loop from 1 to M and do following:
  // Randomize A[i] in scope [1..10]
  // Output A[i] for user to see
  randomize;
  writeln('Random generated array:');
  for i := 1 to M do
  begin
    A[i] := random(10) + 1;
    write(A[i], ' ');
